...
RakNet::BitStream* ConvertToBitStream(custom cu)
{
	RakNet::BitStream *bsOut = new RakNet::BitStream();
	//bsOut->Write(cu);
	bsOut->Write( (RakNet::MessageID)ID_USER_PACKET_ENUM);
	bsOut->Write( cu.clientID);
	bsOut->Write( cu.moveType);
	bsOut->Write(cu.x);
	bsOut->Write(cu.y);
	return bsOut;
}
...
void Setup()
{
	...
	socketDescriptor = RakNet::SocketDescriptor(atoi(clientPort),0);
...
char DecideMoveType(char input)
{
	if(playerType == 0)
	{
		switch (input)
...
void SendMessage()
{
	...
	{
		custom cust;
		cust.clientID = playerType;
		cust.moveType = DecideMoveType(message[0]);
		...
		cust.y = atoi(temp);
		cust.typeID = ID_USER_PACKET_ENUM;
		RakNet::BitStream *bsOut = ConvertToBitStream(cust);
...
int Input()
{
		if (kbhit())
		{
			...
			//format wanted *char*xx*space*yy
...
void HandleIncomingMessage()
{
	// We got a packet, get the identifier with our handy function
			...
			case ID_USER_INVALID_MOVE:
				printf("SERVER - INVALID MOVE");
				break;
			case ID_USER_WAIT_MOVE:
				printf("Please wait until Everyone has made a move");
				break;
			case ID_USER_PACKET_ENUM:
				printf("Client %i : %s", p->data[sizeof(ID_USER_PACKET_ENUM) + sizeof(int)], p->data[sizeof(ID_USER_PACKET_ENUM) + sizeof(int) + sizeof(char[50]) ]);
				break;
			default:
